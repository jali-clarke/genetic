...
populationWithFitnessSortedDesc config population = do
  populationWithFitness <- Vector.forM population $ \member -> fmap ((,) member) (fitness config member)
...
nextPopulation config populationWithFitness = do
